...
{
#define IRRADIANCE_CONVOLUTION_PASS_SETUP_CONTEXT \
...
#define IRRADIANCE_ATTACHMENT_FORMAT VK_FORMAT_R8G8B8A8_SRGB
    ...
    const VkRenderPass* IrradianceConvolutionPass::GetRenderPass() { return &m_render_pipeline.render_pass; }

    void IrradianceConvolutionPass::Initialize(std::shared_ptr<BaseRenderPassInitInfo> info)
    {
        m_pass_info                   = std::dynamic_pointer_cast<IrradianceConvolutionPassInitInfo>(info);
    ...
    void IrradianceConvolutionPass::UnInit()
    {
        SetupFramebuffers();
        ResetPipeline();
        ResetDescriptorSets();
    ...
    void IrradianceConvolutionPass::Recreate() {}

     void IrradianceConvolutionPass::SetupAttachments()
    {
        IRRADIANCE_CONVOLUTION_PASS_SETUP_CONTEXT
        ...
        auto& irradiance_attachment = framebuffer.attachments[0];

        irradiance_attachment = std::make_shared<Texture>();
        irradiance_attachment->InitAttachment(context,
                                              width,
                                              height,
                                              1,
    ...
    void IrradianceConvolutionPass::ResetAttachments()
    {
    ...
    void IrradianceConvolutionPass::SetupShaderResources()
    {
        IRRADIANCE_CONVOLUTION_PASS_SETUP_CONTEXT

        m_ubo = std::make_shared<Buffer>();
        m_ubo->Allocate(context,
                        sizeof(IrradianceConvolutionPassUniformBuffer),
                        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                        VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                        VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
    }

    void IrradianceConvolutionPass::ResetShaderResources() { m_ubo = nullptr; }

    void IrradianceConvolutionPass::UpdateUniformBuffer(const IrradianceConvolutionPassUniformBuffer& uniform_buffer)
    {
        m_ubo->map();
    ...
    void IrradianceConvolutionPass::SetupPipeline()
    {
        IRRADIANCE_CONVOLUTION_PASS_SETUP_CONTEXT

        std::vector<VkAttachmentDescription> attachment_descs(1);
        // irradiance
        attachment_descs[0].format         = IRRADIANCE_ATTACHMENT_FORMAT;
        ...
        VkAttachmentReference             irradiance_ref {0, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL};
        subpasses[0].pipelineBindPoint    = VK_PIPELINE_BIND_POINT_GRAPHICS;
        subpasses[0].colorAttachmentCount = 1;
        subpasses[0].pColorAttachments    = &irradiance_ref;
        ...
        auto vert_code =
            readBinaryFile("G:\\Visual Studio Projects\\VkEngineDemo\\engine\\shaders\\generated\\irradiance_convolution_vert.spv");
        ...
        auto frag_code =
            readBinaryFile("G:\\Visual Studio Projects\\VkEngineDemo\\engine\\shaders\\generated\\irradiance_convolution_frag.spv");
    ...
    void IrradianceConvolutionPass::ResetPipeline()
    {
        IRRADIANCE_CONVOLUTION_PASS_SETUP_CONTEXT
    ...
    void IrradianceConvolutionPass::SetupDescriptorSets()
    {
        IRRADIANCE_CONVOLUTION_PASS_SETUP_CONTEXT
    ...
    void IrradianceConvolutionPass::SyncDescriptorSets()
    {
        IRRADIANCE_CONVOLUTION_PASS_SETUP_CONTEXT
    ...
    void IrradianceConvolutionPass::SetupFramebuffers()
    {
        IRRADIANCE_CONVOLUTION_PASS_SETUP_CONTEXT
        ...
        std::vector<VkImageView> attachments {irradiance_attachment->getView()};
    ...
    void IrradianceConvolutionPass::ResetFramebuffers()
    {
        auto& context = m_pass_info->render_context.vk_context;
    ...
    void IrradianceConvolutionPass::ResetDescriptorSets() { m_desc_tracker = nullptr; }

    void IrradianceConvolutionPass::PreDrawSetup() {}
    void IrradianceConvolutionPass::PostDrawCallback() {}
    void IrradianceConvolutionPass::Draw()
    {
        IRRADIANCE_CONVOLUTION_PASS_SETUP_CONTEXT
        uint32_t width = m_pass_info->width, height = m_pass_info->height;
